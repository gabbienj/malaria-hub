...
option_list = list(
  ...
  make_option(c("-m", "--metadata"), type = "character", default = NULL,
              help = "Full directory path to metadata file",
              metavar = "character"),
  make_option(c("-c", "--category"), type = "character", default = NULL,
              help = "Name of country/region e.g., Indonesia or Southern_SEA",
              metavar = "character"),
  make_option(c("--label_category"), type = "character", default = "country",
              help = "Label name in metadata for category column e.g., country or region",
              metavar = "character"),
  ...
  make_option(c("--remove_chr"), type = "character", default = NULL,
              help = "Chromosomes to remove e.g., Pf3D7_API_v3,Pf_M76611",
              metavar = "character"),
  make_option("--regex_chr", type = "character", default = "(.*?)_(.+)_(.*)",
              help = "Regex pattern for chromosome detection. Default matches Pf3D7_01_v3 or PvP01_01_v1",
              metavar = "character"),
...
if (!is.null(category)) {
  if (length(category) == 1 & category %in% av_category) {
    ...
    stop("Can not find " %+% bgBlue(category) %+% " category. Exiting...\n")
  }
} else {
...
if (!is.null(rm_chr)) {
  ...
  message("No chromosomes removed. Apicoplast (API) and mitochondria (MIT) should be removed!")
}
